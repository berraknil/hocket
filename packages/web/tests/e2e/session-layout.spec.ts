...
test.describe("Session Layout - Header and Footer", () => {
  ...
  test("should navigate to landing page when clicking logo", async ({
    page,
  }) => {
    await page.goto("/s/test-layout");
    await page.waitForLoadState("networkidle");
...
test.describe("Session Layout - Constrained Width", () => {
  test("should have constrained editor container with max-w-7xl", async ({
    page,
  }) => {
...
test.describe("Session Layout - Responsive", () => {
  ...
  test("should show main content area below header with proper padding", async ({
    page,
  }) => {
