...
test.describe('Session Collaboration', () => {
  ...
  test('should sync code across page reload', async ({ page }) => {
    ...
    // Code should persist
    await expect(page.locator('text=// synced code test')).toBeVisible({ timeout: 10000 });
  });
...
test.describe('Multiple Users Simulation', () => {
  test('should handle same session from two pages', async ({ context }) => {
    ...
    await expect(page2.locator('text=// shared session test')).toBeVisible({ timeout: 15000 });
  ...
  test('should show different usernames for different pages', async ({ context }) => {
    ...
    const username1 = await page1.evaluate(() => localStorage.getItem('username'));
    const username2 = await page2.evaluate(() => localStorage.getItem('username'));
...
test.describe('Session URL Sharing', () => {
  ...
  test('should require authentication to share session URL', async ({ page }) => {
  ...
  test('should redirect to sign-in when loading shared session without auth', async ({ page }) => {
    // Create a session URL with hash parameters (shared code)
    const sessionUrl = '/s/test-hash#targets=strudel&c0=' + encodeURIComponent(btoa('sound("bd")'));
