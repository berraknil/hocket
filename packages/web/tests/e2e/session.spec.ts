...
test.describe('Session Page', () => {
  ...
  test('should show two panes by default (strudel and hydra)', async ({ page }) => {
  ...
  test('should show sample code in new session', async ({ page }) => {
    ...
    await expect(page.locator('text=From Strudel Recipes').first()).toBeVisible({ timeout: 10000 });
  });
...
test.describe('Session Save/Download Functionality', () => {
  ...
  test('should show save option in command dialog when authenticated', async ({ page }) => {
    ...
    await expect(page.locator('[cmdk-item]:has-text("Save to ATproto")')).toBeVisible({ timeout: 5000 });
  });
