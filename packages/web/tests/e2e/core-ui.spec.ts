...
test.describe('Landing Page', () => {
  test('should display the landing page with new tagline', async ({ page }) => {
    ...
    await expect(page.locator('text=Collaborative live coding in the browser, built on AtProto')).toBeVisible();
  });
  ...
  test('should display footer with attributions', async ({ page }) => {
    ...
    await expect(page.locator('footer a[href="https://flok.cc"]')).toContainText('Flok');
    await expect(page.locator('footer a[href="https://strudel.cc"]')).toContainText('Strudel');
    await expect(page.locator('footer a[href="https://hydra.ojack.xyz"]')).toContainText('Hydra');
    await expect(page.locator('footer a[href="https://tidalcycles.org"]')).toContainText('Tidal Cycles');
  });
  ...
  test('should show Sign In link in header when not authenticated', async ({ page }) => {
    await page.goto('/');

    await expect(page.locator('header a[href="/auth/sign-in"]')).toBeVisible();
  });

  test('should show Start Jamming and Sign In buttons when not authenticated', async ({ page }) => {
    await page.goto('/');

    await expect(page.locator('main a:has-text("Start Jamming")')).toBeVisible();
    await expect(page.locator('main a[href="/auth/sign-in"]:has-text("Sign In")')).toBeVisible();
  });

  test('should show Go to Dashboard button when authenticated', async ({ page }) => {
    const mockSession = {
      did: 'did:plc:test123',
    ...
    await expect(page.locator('main a[href="/dashboard"]:has-text("Go to Dashboard")')).toBeVisible({ timeout: 10000 });
  });

  test('should not show Sign In in header when authenticated', async ({ page }) => {
    const mockSession = {
      did: 'did:plc:test123',
      handle: 'test.bsky.social',
      accessJwt: 'mock-jwt',
    ...
    await expect(page.locator('header a[href="/auth/sign-in"]')).not.toBeVisible();
  });
  ...
  test('should navigate to session when clicking Start Jamming', async ({ page }) => {
    await page.goto('/');
...
test.describe('Dashboard', () => {
  ...
  test('should display Hocket link that goes to landing page', async ({ page }) => {
    await page.goto('/');
    await page.evaluate((session) => {
      localStorage.setItem('hocket-session', JSON.stringify(session));
    }, mockSession);
...
test.describe('Authentication Flow', () => {
  ...
  test('should show loading state with spinner during authentication', async ({ page }) => {
    await page.goto('/auth/sign-in');
  ...
  test('should display help text with link to create account', async ({ page }) => {
    await page.goto('/auth/sign-in');
    
    await expect(page.locator('text=Don\'t have an account?')).toBeVisible();
    await expect(page.locator('a[href="https://bsky.app"]')).toContainText('Create one on Bluesky');
  });
