...
test.describe('Editor Functionality', () => {
  ...
  test('should persist editor content during session', async ({ page }) => {
    ...
    await expect(page.locator('text=// persistent test data')).toBeVisible({ timeout: 10000 });
  });
});
...
test.describe('Editor Settings', () => {
  test('should open editor settings from command palette', async ({ page }) => {
    ...
    await expect(page.locator('text=Line Numbers')).toBeVisible({ timeout: 5000 });
  });
...
test.describe('Code Evaluation', () => {
  ...
  test('should evaluate code when clicking evaluate button', async ({ page }) => {
